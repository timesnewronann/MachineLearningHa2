...
# three columns: MATH SAT, VER
# B SAT, UNI. GPA  # convert frame to matrix
sat = download_data('ha2/sat.csv', [1, 2, 4]).values

################ PLACEHOLDER2 #start##########################
# Normalize data
# Mean normalization
sat_mean = np.mean(sat, axis=0)
sat_std = np.std(sat, axis=0)
sat = (sat - sat_mean) / sat_std
################ PLACEHOLDER2 #end##########################
